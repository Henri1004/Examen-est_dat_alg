...
class Room():
    ...
    def is_occupied(self):
        if self.room_state == "Ocupada":
            return True
        elif self.room_state == "Desocupada":
            return False

    def check_in(self):
        if self.room_number in self._habitaciones_ocupadas:
            return "La habitación ya está ocupada."
        if self.get_room_state() == "Desocupada":
    ...
    def check_out(self):
        if self.room_state == "Desocupada":
            return "No se puede hacer check out, porque la habitacion ya esta vacia"
        elif self.room_state == "Ocupada":
